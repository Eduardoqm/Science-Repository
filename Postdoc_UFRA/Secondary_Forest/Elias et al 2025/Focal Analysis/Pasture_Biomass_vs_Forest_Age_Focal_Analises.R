#ESA Biomass by Forest Age - Pasture Delta (Focal)
#E.Q.Marques 17-08-2026
...
save_delta_raster <- function(esa_rast, output_file) {
  ...
  esa_past <- ifel(is.na(past), NA, esa_rast)
  #esa_f   <- focal(esa_past, w=21, fun=median, na.rm=TRUE, na.policy="only")
  esa_f   <- focal(esa_past, w=21, fun=quantile, probs = 0.95, na.rm=TRUE, na.policy="only")
...
#Focal
save_delta_raster(esa, "Delta_AGB_Forest_age_Pasture.tif")
delta_esa  <- rast("Delta_AGB_Forest_age_Pasture.tif")

#Dataframe
process_lst_block(esa, delta_esa, "Annual", "Pasture_AGB_age_full.csv")
...
#Summarizing data --------------------------------------------------------------
df = read_csv("Pasture_AGB_age_full.csv")
...
final_df2 = df %>%
  ...
  group_by(age, cond) %>%
  summarise(
    delta_esa = mean(delta_esa, na.rm = TRUE),
    sf_perc   = mean(sf_perc, na.rm = TRUE),
    n = n()) %>%
  filter(n > 200, age < 250)

write.csv(final_df2, "Pasture_AGB_age_full_C.csv", row.names = FALSE)



ggplot(final_df2, aes(x = age, y = delta_esa))+
  geom_point()+
  geom_smooth()