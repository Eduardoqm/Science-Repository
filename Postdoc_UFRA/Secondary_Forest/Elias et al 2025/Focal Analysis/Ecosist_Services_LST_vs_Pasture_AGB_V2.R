#LST Pasture by ESA Biomass (Focal)
...
save_delta_raster <- function(lst_rast, output_file) {
  ...
  lst_pri <- ifel(is.na(pasto), NA, lst_rast)
...
process_lst_block <- function(lst_rast,Delta_rast, cond_name, output_file, nlines_block = 100) {
  ...
  #Create a empity CSV
  write.csv(
    data.frame(agb=NA, delta_lst=NA, sf_perc=NA, cond=NA)[0,],
    output_file,
    row.names=FALSE
  )
  
  nrows <- nrow(lst_rast) #Lines count
  esa2 <- ifel(is.na(lst_rast), NA, esa)#AGB mask
...
#Focal
save_delta_raster(lst_year, "Delta_LST_Pasture_AGB_Annual_2022.tif")
save_delta_raster(lst_dry,  "Delta_LST_Pasture_AGB_Dry_2022.tif")
save_delta_raster(lst_wet,  "Delta_LST_Pasture_AGB_Rainy_2022.tif")

Delta_year  <- rast("Delta_LST_Pasture_AGB_Annual_2022.tif")  
Delta_dry  <- rast("Delta_LST_Pasture_AGB_Dry_2022.tif")  
Delta_wet  <- rast("Delta_LST_Pasture_AGB_Rainy_2022.tif")  

#Dataframe
process_lst_block(lst_year, Delta_year, "Annual", "LST_AGB_Annual_full.csv")
process_lst_block(lst_dry, Delta_dry,  "Dry Season", "LST_AGB_Dry_full.csv")
process_lst_block(lst_wet,Delta_wet,  "Rainy Season", "LST_AGB_Rainy_full.csv")
...
#Summarizing data --------------------------------------------------------------
year_df <- read_csv("LST_AGB_Annual_full.csv")
dry_df <- read_csv("LST_AGB_Dry_full.csv")
rainy_df <- read_csv("LST_AGB_Rainy_full.csv")
...
write.csv(final_df2, "LST_Pasture_AGB_2022.csv", row.names = FALSE)
