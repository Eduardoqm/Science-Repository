#ESA Biomass by Forest Age - Forest Delta (Focal)
...
#Summarizing data --------------------------------------------------------------
df = read_csv("Forest_Pri_AGB_age_full.csv")

#final_df = df %>%
#  filter(sf_perc >= 70) %>%
#  mutate(age = round(age, 0))

#write.csv(final_df, "Forest_Pri_AGB_age_full_B.csv", row.names = FALSE)
...
write.csv(final_df2, "Forest_Pri_AGB_age_full_C.csv", row.names = FALSE)
