...
#Means from classes ------------------------------------------------------------
lst_cur = rast("LST_Annual_current.tif")
...
dlt_lst_p = rast("Pasture_Delta_LST_annual.tif")
scf = rast("MB_Forest_age_70m.tif")
...
dlt_lst_p2 = ifel(is.na(scf), NA, dlt_lst_p)
plot(dlt_lst_p2)

dlt_lst_p3 = ifel(scf > 14, dlt_lst_p2, NA) #Filtering age
plot(dlt_lst_p3)

scn_p = lst_cur+dlt_lst_p3
scn_p2 = ifel(sc_perc > 69, scn_p, NA) #Filtering percentages
scn_p3 = ifel(is.na(scn_p2), lst_cur, scn_p2)


mean(values(lst_cur),na.rm=T)
mean(values(scn_p3),na.rm=T)
