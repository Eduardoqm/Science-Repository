...
#Crop to sample
index_a100=crop(npcri, a_100); index_b100=crop(npcri, b_100)
index_c100=crop(npcri, c_100); index_d100=crop(npcri, d_100)

index_acore=crop(npcri, a_core); index_bcore=crop(npcri, b_core)
index_ccore=crop(npcri, c_core); index_dcore=crop(npcri, d_core)
...
npcri_100 <- ndvi_100[,c(1,2,4,6,8)]
npcri_100$data = substr(npcri_100$data, 18, 25)
...
npcri_core <- ndvi_core[,c(1,2,4,6,8)]
npcri_core$data = substr(npcri_core$data, 18, 25)
...
npcri_md2 <- ndvi_md[-c(19),] 

setwd('C:/Users/Eduardo Q Marques/Documents/My Jobs/Doutorado/Climate data')
spi <- read.csv("SPI_Area1.csv", header = TRUE, sep = ",")

#B1yr
crt_lm<-lm(npcri_md2$controle~spi$d) #(Y~X)
b3yr_lm<-lm(npcri_md2$b3yr~spi$d)
b1yr_lm<-lm(npcri_md2$b1yr~spi$d)
d_lm<-lm(npcri_md2$plot_d~spi$d)
summary(crt_lm)
summary(b1yr_lm)
summary(b3yr_lm)
summary(d_lm)
...
gg = as.data.frame(cbind(npcri_md2$controle, npcri_md2$b3yr, npcri_md2$b1yr, npcri_md2$plot_d, spi$d))
colnames(gg) = c('control', 'b3yr', 'b1yr', 'plot_d', 'spi')
gg <- melt(gg, id.vars="spi")
colnames(gg) = c('SPI', 'Plot', 'NPCRI')

ggplot(gg, aes(SPI,NPCRI, col=Plot))+ 
  geom_point()+
  stat_smooth(method = "lm", se=FALSE)+
  theme_minimal()+
  theme(legend.position = 'top')

gg = as.data.frame(cbind(npcri_md2$controle, npcri_md2$b3yr, npcri_md2$b1yr, npcri_md2$plot_d, mcwd$mcwd))
colnames(gg) = c('control', 'b3yr', 'b1yr', 'plot_d', 'mcwd')
gg <- melt(gg, id.vars="mcwd")
colnames(gg) = c('MCWD', 'Plot', 'NPCRI')

ggplot(gg, aes(MCWD,NPCRI, col=Plot))+ 
  geom_point()+
  stat_smooth(method = "lm", se=FALSE)+
  theme_minimal()+
  theme(legend.position = 'top')
