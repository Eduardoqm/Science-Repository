...
#Crop to sample
index_a100=crop(grnd, a_100); index_b100=crop(grnd, b_100)
index_c100=crop(grnd, c_100); index_d100=crop(grnd, d_100)

index_acore=crop(grnd, a_core); index_bcore=crop(grnd, b_core)
index_ccore=crop(grnd, c_core); index_dcore=crop(grnd, d_core)
...
grnd_100 <- ndvi_100[,c(1,2,4,6,8)]
grnd_100$data = substr(grnd_100$data, 18, 25)
...
grnd_core <- ndvi_core[,c(1,2,4,6,8)]
grnd_core$data = substr(grnd_core$data, 18, 25)
...
grnd_md2 <- ndvi_md[-c(19),] 

setwd('C:/Users/Eduardo Q Marques/Documents/My Jobs/Doutorado/Climate data')
spi <- read.csv("SPI_Area1.csv", header = TRUE, sep = ",")

#B1yr
crt_lm<-lm(grnd_md2$controle~spi$d) #(Y~X)
b3yr_lm<-lm(grnd_md2$b3yr~spi$d)
b1yr_lm<-lm(grnd_md2$b1yr~spi$d)
d_lm<-lm(grnd_md2$plot_d~spi$d)
summary(crt_lm)
summary(b1yr_lm)
summary(b3yr_lm)
summary(d_lm)
...
gg = as.data.frame(cbind(grnd_md2$controle, grnd_md2$b3yr, grnd_md2$b1yr, grnd_md2$plot_d, spi$d))
colnames(gg) = c('control', 'b3yr', 'b1yr', 'plot_d', 'spi')
gg <- melt(gg, id.vars="spi")
colnames(gg) = c('SPI', 'Plot', 'GRND')

ggplot(gg, aes(SPI,GRND, col=Plot))+ 
  geom_point()+
  stat_smooth(method = "lm", se=FALSE)+
  theme_minimal()+
  theme(legend.position = 'top')

gg = as.data.frame(cbind(grnd_md2$controle, grnd_md2$b3yr, grnd_md2$b1yr, grnd_md2$plot_d, mcwd$mcwd))
colnames(gg) = c('control', 'b3yr', 'b1yr', 'plot_d', 'mcwd')
gg <- melt(gg, id.vars="mcwd")
colnames(gg) = c('MCWD', 'Plot', 'GRND')

ggplot(gg, aes(MCWD,GRND, col=Plot))+ 
  geom_point()+
  stat_smooth(method = "lm", se=FALSE)+
  theme_minimal()+
  theme(legend.position = 'top')