...
#Crop to sample
index_a100=crop(ndmi, a_100); index_b100=crop(ndmi, b_100)
index_c100=crop(ndmi, c_100); index_d100=crop(ndmi, d_100)

index_acore=crop(ndmi, a_core); index_bcore=crop(ndmi, b_core)
index_ccore=crop(ndmi, c_core); index_dcore=crop(ndmi, d_core)
...
ndmi_100 <- ndvi_100[,c(1,2,4,6,8)]
ndmi_100$data = substr(ndmi_100$data, 18, 25)
...
ndmi_core <- ndvi_core[,c(1,2,4,6,8)]
ndmi_core$data = substr(ndmi_core$data, 18, 25)
...
ndmi_md2 <- ndvi_md[-c(19),] 

setwd('C:/Users/Eduardo Q Marques/Documents/My Jobs/Doutorado/Climate data')
spi <- read.csv("SPI_Area1.csv", header = TRUE, sep = ",")

#B1yr
crt_lm<-lm(ndmi_md2$controle~spi$d) #(Y~X)
b3yr_lm<-lm(ndmi_md2$b3yr~spi$d)
b1yr_lm<-lm(ndmi_md2$b1yr~spi$d)
d_lm<-lm(ndmi_md2$plot_d~spi$d)
summary(crt_lm)
summary(b1yr_lm)
summary(b3yr_lm)
summary(d_lm)
...
gg = as.data.frame(cbind(ndmi_md2$controle, ndmi_md2$b3yr, ndmi_md2$b1yr, ndmi_md2$plot_d, spi$d))
colnames(gg) = c('control', 'b3yr', 'b1yr', 'plot_d', 'spi')
gg <- melt(gg, id.vars="spi")
colnames(gg) = c('SPI', 'Plot', 'NDMI')

ggplot(gg, aes(SPI,NDMI, col=Plot))+ 
  geom_point()+
  stat_smooth(method = "lm", se=FALSE)+
  theme_minimal()+
  theme(legend.position = 'top')

gg = as.data.frame(cbind(ndmi_md2$controle, ndmi_md2$b3yr, ndmi_md2$b1yr, ndmi_md2$plot_d, mcwd$mcwd))
colnames(gg) = c('control', 'b3yr', 'b1yr', 'plot_d', 'mcwd')
gg <- melt(gg, id.vars="mcwd")
colnames(gg) = c('MCWD', 'Plot', 'NDMI')

ggplot(gg, aes(MCWD,NDMI, col=Plot))+ 
  geom_point()+
  stat_smooth(method = "lm", se=FALSE)+
  theme_minimal()+
  theme(legend.position = 'top')
