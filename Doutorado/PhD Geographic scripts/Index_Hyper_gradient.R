...
b3yr_seq = seq(2,30, by=3)
for (x in b3yr_seq) {
  plot(index[[12]], main = 'Progress (EXTRACT)')
  plot(area1[x,], add=TRUE)
  b3yr <- extract(index, area1[x,])
  b <- melt(b3yr)
  colnames(b) = c('id', 'index', 'controle', 'l1')
  b$gradient = c(x)
  bloco_b = as.data.frame(rbind(bloco_b, b))
}
...
a_900_1000 <- extract(index, area1[3,])#PLot A
b_900_1000 <- extract(index, area1[2,])#PLot B
c_900_1000 <- extract(index, area1[1,])#PLot c
