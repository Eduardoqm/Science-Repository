...
#Extrair valor dos pixels
crtdf <- extract(ndwi, crt)
b1yrdf <- extract(ndwi, b1yr)
b3yrdf <- extract(ndwi, b3yr)
...
#Extrair valor dos pixels
crtdf <- extract(grnd, crt)
b1yrdf <- extract(grnd, b1yr)
b3yrdf <- extract(grnd, b3yr)
...
ggplot(gg, aes(Data,Refl, col=Parcela))+ 
  ...
  geom_point()+
  stat_smooth(aes(group=Parcela), method = "loess", formula = y ~ x, size = 0.5, alpha = 0.15)+
  labs(fill= "Plot",x="Ano",y="Fogo - Controle (% diferença no GRND)")+
  geom_hline(yintercept = 0, color = "gray", linetype = "dashed")+
  
  annotate("text", x = c(8.5, 11.5, 15.5), y = 2.5, label = "Seca")+
  annotate("segment", x = c(8.5, 11.5, 15.5), y = 2, 
           xend = c(8.5, 11.5, 15.5), yend = 0,
           arrow = arrow(angle = 20, length = unit(2, "mm"), type = "closed"))+
  
  annotate("text", x = 2.5, y = -8, label = "Pré-fogo", colour = "darkgreen")+
  annotate("rect", xmin = 1, xmax = 5, ymin = -8.6, ymax = -9, alpha = 0.9, fill = "darkgreen")+
  
  annotate("text", x = 8.5, y = -8, label = "Experimento de fogo", colour = "red")+
  annotate("rect", xmin = 5, xmax = 12, ymin = -8.6, ymax = -9, alpha = 0.9, fill = "red")+
  annotate("text", x = c(5.5, 8.5, 11.5), y = -8.7, label = "xx", colour = "yellow")+
  annotate("text", x = c(6.5, 7.5, 10.5), y = -8.7, label = "x", colour = "yellow")+
  
  annotate("text", x = 15.5, y = -8, label = "Periodo de recuperação", colour = "darkblue")+
  annotate("rect", xmin = 12, xmax = 19, ymin = -8.6, ymax = -9, alpha = 0.9, fill = "darkblue")+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 90),legend.position = c(0.9, 0.3))
