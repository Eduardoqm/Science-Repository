...
chibar_plot = ggplot(chibar2, aes(quant, value, fill = Dataset, linetype = Dataset))+
  ...
  xlim(0.5, 1)+
  theme_bw()+
  theme(legend.position = c(.7,.2)); chibar_plot
...
chis2 = ggarrange(chi_plot, chibar_plot, ncol = 1); chis2
chis3 = ggarrange(chis, chis2, ncol = 2); chis3

ggsave(filename = "WS-Prec_chi_chibar_Facet.png", plot = chis3,
       path = "C:/Users/Eduardo Q Marques/Documents/Research/Doutorado/Capitulo2/Figuras/Wind Speed vs Precipitation", width = 25, height = 20, units = "cm", dpi = 300)
