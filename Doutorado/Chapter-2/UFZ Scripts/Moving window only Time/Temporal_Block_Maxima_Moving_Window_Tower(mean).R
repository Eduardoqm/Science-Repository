#---------------------------------------------------------------------------------------------------
# Temporal Block Maxima WS (MEAN) and Precipitation of Control Tower and Darro Station (Moving Window)
#
# Eduardo Q Marques 01-08-2022    Updated: 09-08-2022
#--------------------------------------------------------------------------------------------------

# Description: This script is the multi temporal block by 1, 3 and 5 days. For the "Mean" WS from
# Control Tower and Precipitation from Darro station. ATTENTION! It's not for MAXIMUM WS!!!
...
tower = tower %>% select(datetime, y, m, d, wind_speed)
...
raw = ggplot(dfb, aes(x=Prec, y=Wind))+
  ...
  labs( x = "Maximum Precipitation (mm)", y = "Wind Speed (m/s)", title = "Raw Data from Tower/Darro Station")+
  scale_color_manual(values = eqm)+
  ylim(0, 10)+
  theme_bw()+
  theme(legend.position = c(30, 30))

ggsave(filename = "Tower-Darro_raw_WS-Prec.png", plot = raw,
...
#multd = multd %>% filter(Wind >= 8) #Limiar of blowdown suggested by Marra
...
multd_plot = ggplot(multd, aes(x=Prec, y=Wind))+
  ...
  labs( x = "Maximum Precipitation (mm)", y = "Wind Speed (m/s)", title = "Tower/Darro Station - Temporal Block Maxima")+
...
ggsave(filename = "Tower_WS-Prec_spatial_temporal_max_1-3-5days.png", plot = multd_plot,
...
test = ggplot(chis, aes(quant, value, fill = window, linetype = window))+
  ...
  labs(x = "Quantile theshold q", y = "Chi", title = "Tail Dependence (Tower)")+
...
ggsave(filename = "Tower_CHI_WS-Prec_spatial_temporal_max_1-3-5days.png", plot = test,
path = "/home/queirozm/eqm_eth_ufz/Temp_figures/Move_window",
