...
multd_plot = ggplot(multd, aes(x=Prec, y=Wind))+
  ...
  labs( x = "Maximum Precipitation (mm)", y = "Wind Speed (m/s)", title = "a) Darro Station")+
  scale_color_manual(values = c("orange", "blue", "red"))+
  scale_fill_manual(values = c("orange", "blue", "red"))+
  ylim(0, 10)+
  theme_bw()+
  theme(legend.position = c(30, 0.50))
...
test = ggplot(chis, aes(quant, value, fill = window, linetype = window))+
  ...
  labs(x = "Quantile theshold q", y = "Chi", title = "c) Tail Dependence (Darro Station)")+
