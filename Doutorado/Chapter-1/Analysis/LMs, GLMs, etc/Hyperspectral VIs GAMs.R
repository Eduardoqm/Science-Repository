...
panel = ggplot(hy2, aes(x=Year, y=Valor, color = Treatment))+
  geom_smooth(aes(group=Treatment), alpha = 0.3, size = 1, method = "gam", formula = y ~ poly(x, 5))+
  geom_vline(xintercept = 2004, linetype = "dashed", size = 1, col = "gray")+
  geom_vline(xintercept = 2011, linetype = "dashed", size = 1, col = "gray")+
  ...
  scale_x_continuous(breaks = seq(2004, 2012, by = 3))+
  theme_minimal()+
  labs(y = NULL)+
...
#    width = 13, height = 18, units =  "cm", dpi = 300)
