...
#PV
pv = df %>% 
  filter(class == "Photosynthetic Vegetation")
...
a = ggplot(bs3, aes(x=value, y=id, fill=plot))+
...
a = ggplot(pv3, aes(x=value, y=id, fill=plot))+
  geom_density_ridges(alpha = 0.5)+
  facet_wrap(~plot, scales = "free")+
  theme_minimal()+
  theme(panel.border = element_rect(colour = "gray", fill=NA, size=1))
